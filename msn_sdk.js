  ...
  adinplayLoadPromise = new Promise(async (resolve) => {
    ...
    try {
      ...
      await new Promise((tagResolve, tagReject) => {
        ...
        tagScript.src = '//api.adinplay.com/libs/aiptag/pub/LGP/veck.io/tag.min.js';
...
let _banner728x90State = { visible: false, position: 0 };
...
// Map Unity banner positions for 320x50 (mapping 728x90 requests to this)
const UNITY_TO_MSN_POSITION_320x50 = {
  1: 'top:320x50',       // TopCenter
  4: 'bottom:320x50',    // BottomCenter
  8: 'left:320x50',      // MiddleLeft
  9: 'right:320x50'      // MiddleRight
};
...
async function processBannerPlacement() {
  ...
  console.log('[msn_sdk.js] 728x90 state:', _banner728x90State);

  // If all are hidden, hide MSN ad
  if (!_banner300x250State.visible && !_banner300x600State.visible && !_banner728x90State.visible) {
    console.log('[msn_sdk.js] All banners hidden, hiding MSN ad');
    await hideMsnDisplayAd();
    return;
  }

  // If 728x90 is visible, place MSN ad in 320x50 slot
  if (_banner728x90State.visible) {
    const position = _banner728x90State.position;
    const msnPlacement = UNITY_TO_MSN_POSITION_320x50[position];

    if (msnPlacement) {
      console.log('[msn_sdk.js] 728x90 visible, placing MSN 320x50 ad in 728x90 slot:', msnPlacement);
      await showMsnDisplayAd(msnPlacement, 1, position);
    } else {
      // Position not supported by MSN, try AdinPlay fallback
      console.log('[msn_sdk.js] 728x90 position', position, 'not supported by MSN 320x50, trying AdinPlay fallback');
      await showAdinplayBannerFallback(1, position);
    }
    return;
  }
...
window.SDK = {
  ...
  async SetBanner(bannerType, bannerPosition) {
    // Accept 300x250 (type 0), 728x90 (type 1), and 300x600 (type 2)
    if (bannerType !== 0 && bannerType !== 1 && bannerType !== 2) {
    ...
    if (bannerType === 0) {
      ...
      // 728x90 state
      _banner728x90State.visible = bannerPosition !== 0;
      _banner728x90State.position = bannerPosition;
      console.log('[msn_sdk.js] 728x90 state updated:', _banner728x90State);
